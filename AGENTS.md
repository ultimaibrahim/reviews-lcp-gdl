...
- **Protocolo de Aprobación de Versionamiento**: Parches `x.x.1` tienen autonomía automática. Cambios `x.1.x` (MINOR) y `1.x.x` (MAJOR / Épocas) OBLIGATORIAMENTE requieren presentar justificación técnica a Ibrahim y recibir aprobación explícita antes de modificar la versión o realizar commits.
- **Escalabilidad Total e Incremental (CRÍTICO)**: Todo nuevo desarrollo, refactor o feature debe construirse de forma modular, extensible y desacoplada. Queda estrictamente prohibido implementar patrones rígidos que requieran rehacer o reescribir la aplicación desde cero al agregar múltiples características nuevas. Los cambios deben integrarse como capas acumulativas sin romper funcionalidades previas.
